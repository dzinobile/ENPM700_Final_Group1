...
class MyRobotDriver:
    def init(self, webots_node, properties):
        ...
        # Lidar device
        self.__lidar = None
        self.__lidar_frame_id = None
        try:
            # Use the actual device name from Webots, for example "lidar1"
            # If you used a different name in the world file, this must match.
            self.__lidar = self.__robot.getDevice("lidar1")
            self.__lidar.enable(self.__timestep)

            # TF frame for lidar, namespaced by robot
            self.__lidar_frame_id = f"{self.__robot_name}/{self.__lidar.getName()}"
        except Exception:
            # No lidar on this robot
            self.__lidar = None
        ...
        if self.__lidar is not None:
            ...
            self.__node.get_logger().warn(
                "No lidar device named 'lidar1' found. LaserScan will not be published."
            )
    ...
    def _publish_scan(self, stamp):
        ...
        scan.header.frame_id = self.__lidar_frame_id
