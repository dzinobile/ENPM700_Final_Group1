...
ExploreState::ExploreState()
    : step_length_(0.5), step_increment_(1), x_(0), y_(0), count_(0),
      map_width_(20.0), map_height_(20.0), goal_active_(false),
      goal_reached_(false) {
...
void ExploreState::sendNextGoal(SheepdogNode &context) {
  ...
  RCLCPP_INFO(context.get_logger(),
              "EXPLORE: Sending goal (%f, %f), step_length: %f", clamped_x,
              clamped_y, step_length_);
  ...
  send_goal_options.goal_response_callback = std::bind(
      &ExploreState::goalResponseCallback, this, std::placeholders::_1);

  send_goal_options.result_callback =
      std::bind(&ExploreState::goalResultCallback, this, std::placeholders::_1);
...
void ExploreState::goalResponseCallback(
  ...
  if (!goal_handle) {
    ...
    RCLCPP_INFO(rclcpp::get_logger("ExploreState"), "goal accepted by server");
  }
}
...
bool ExploreState::isPointInBounds(double x, double y) const {
  return (x >= map_min_x_ && x <= map_max_x_ && y >= map_min_y_ &&
          y <= map_max_y_);
}
...
void ExploreState::clampGoalToBounds(double &x, double &y) {
  ...
  if (x < map_min_x_)
    x = static_cast<double>(map_min_x_);
  if (x > map_max_x_)
    x = static_cast<double>(map_max_x_);
  if (y < map_min_y_)
    y = static_cast<double>(map_min_y_);
  if (y > map_max_y_)
    y = static_cast<double>(map_max_y_);

  if (x != original_x || y != original_y) {
    RCLCPP_INFO(rclcpp::get_logger("ExploreState"),
                "Clamped goal from (%f, %f) to (%f, %f)", original_x,
                original_y, x, y);
  }
}
