...
EncircleState::EncircleState()
    : goal_sent_(false), goal_reached_(false), offset_distance_(0.5) {}
...
void EncircleState::sendEncircleGoal(SheepdogNode &context) {
  ...
  try {
    sheep_tf =
        context.tf_buffer_->lookupTransform("map", "sheep", tf2::TimePointZero);
  ...
  send_goal_options.goal_response_callback = std::bind(
      &EncircleState::goalResponseCallback, this, std::placeholders::_1);
  send_goal_options.result_callback = std::bind(
      &EncircleState::goalResultCallback, this, std::placeholders::_1);
...
void EncircleState::goalResponseCallback(
