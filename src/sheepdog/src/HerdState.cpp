...
void HerdState::sendHerdGoal(SheepdogNode &context) {
    ...
    try {
        pen_tf = context.tf_buffer_->lookupTransform(
            "map",
            "pen",
            tf2::TimePointZero
        );
...
void HerdState::goalResponseCallback(const GoalHandleNav::SharedPtr& goal_handle) {
    if (!goal_handle) {
        goal_sent_ = false;
    } else {
...
void HerdState::goalResultCallback(
