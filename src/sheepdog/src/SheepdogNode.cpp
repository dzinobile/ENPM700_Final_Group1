...
SheepdogNode::SheepdogNode()
    : Node("sheepdog_node"), sheep_detection_radius_(1.0),
  ...
  sheep_detection_radius_ =
      this->get_parameter("sheep_detection_radius").as_double();
...
void SheepdogNode::run() { rclcpp::spin(shared_from_this()); }
...
void SheepdogNode::updateSheepDetection() {
  try {
    ...
    if (sheep_detected_ && !was_detected) {
      RCLCPP_INFO(this->get_logger(), "sheep detected %.2f meters",
                  distance_to_sheep_);
